...
2. **建立專用的儲存桶**：
   ...
   cd terraform
   terraform init
   terraform apply -target=google_storage_bucket.terraform_state -var-file="terraform.tfvars"
   ```
...
正式將架構部署至雲端。由於 Kubernetes 官方套件的限制，全新部署時必須分兩階段執行：

1. **第一階段：先建立 GKE 叢集與基礎設施**
   ```bash
   terraform init  # 再次初始化，當系統詢問是否將本機狀態轉移至雲端時，請輸入 yes
   terraform apply -target=google_container_cluster.openclaw_cluster -var-file="terraform.tfvars"
   ```
   *輸入 `yes` 以確認執行。這個過程大約需要 15~20 分鐘。*

2. **第二階段：部署剩餘的 Kubernetes 應用與資源**
   等叢集建立完成後，執行最終的完整部署：
   ```bash
