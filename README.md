...
*   **Secure Web Proxy (SWP):** All outbound traffic generated by the OpenClaw AI Agent (e.g., when it autonomously uses tools to browse the web or crawl APIs) is explicitly routed through Google Cloud Secure Web Proxy via the `HTTPS_PROXY` environment variable.
    *   **Complete Autonomy:** By default, connections are allowed (`allow_all_web`). This preserves the AI's ability to freely explore the internet to solve problems.
    ...
    *   *(Note: While Google Cloud Threat Intelligence is highly effective, it is currently supported only on Cloud Armor, not SWP egress rules. We rely on Model Armor for payload inspection and SWP logs for post-incident auditing.)*
